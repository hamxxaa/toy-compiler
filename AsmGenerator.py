class AsmGenerator:
    ...
    def apply_cmp(self, operator):
        second_register = self.get_register_from_stack()
        first_register = self.get_register_from_stack()

        if first_register in ["esi", "edi"] and second_register == ["esi", "edi"]:
            self.code += "pop esi\n"
            self.code += "push eax\n"
            self.code += "mov eax, [esp+4]\n"
            self.code += "cmp eax, esi\n"
            self.set_lower_register(operator, "eax")
            self.code += "movzx esi, al"
            self.code += "pop eax\n"
            self.code += "mov [esp], esi\n"
            self.outer_stack.append(["stack", None])
            self.stack_size += 1
        else:
            self.code += f"cmp {first_register}, {second_register}\n"
            self.set_lower_register(operator, first_register)
            self.code += f"movzx {first_register}, {self.get_lower(first_register)}\n"
            self.outer_stack.append([first_register, None])
            self.used_registers.add(first_register)
            self.stack_size += 1

    def get_lower(self, register):
        if register == "eax":
            return "al"
        elif register == "ebx":
            return "bl"
    ...
    def set_lower_register(self, operator, register):
        lower_register = self.get_lower(register)
        if operator == "<":
            self.add_to_code(f"setl {lower_register} \n")
        elif operator == ">":
            self.add_to_code(f"setg {lower_register} \n")
        elif operator == "==":
            self.add_to_code(f"sete {lower_register} \n")
        elif operator == "<=":
            self.add_to_code(f"setle {lower_register} \n")
        elif operator == ">=":
            self.add_to_code(f"setge {lower_register} \n")
        elif operator == "!=":
            self.add_to_code(f"setne {lower_register} \n")
